    ...
    <main class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-8">
        ...
        <div v-if="viewMode === 'table'" class="card">
            <DataTable
                ...
                dataKey="id"
                :globalFilterFields="['name', 'description', 'selling_price', 'cost_price', 'stock_quantity', 'supplier', 'status', 'image_url']"
            ...
            >
                ...
                <Column field="selling_price" header="Selling Price" sortable style="width: 25%">
                    <template #filter="{ filterModel }">
                        <InputNumber v-model="filterModel.value" placeholder="Search selling price" :minFractionDigits="0" />
                    </template>
                </Column>

                <Column field="cost_price" header="Cost Price" sortable style="width: 25%">
                    <template #filter="{ filterModel }">
                        <InputNumber v-model="filterModel.value" placeholder="Search cost price" :minFractionDigits="0" />
                    </template>
                </Column>

                <Column field="stock_quantity" header="QTY" sortable style="width: 25%">
                    <template #filter="{ filterModel }">
                        <InputNumber v-model="filterModel.value" placeholder="Search quantity" />
                    </template>
                </Column>

                <Column field="supplier" header="Supplier" sortable style="width: 25%">
                    <template #filter="{ filterModel }">
                        <InputText v-model="filterModel.value" type="text" placeholder="Search supplier" />
                    </template>
                </Column>
                    ...
                    <template #filter="{ filterModel }">
                        <Select v-model="filterModel.value" :options="statuses" placeholder="Select One" showClear>
                            <template #option="slotProps">
                                <Tag :value="slotProps.option ? 'Active' : 'Inactive'" :severity="getSeverity(slotProps.option)" />
                            </template>
                        </Select>
                    </template>
                </Column>
    ...
    confirm.require({
        ...
        accept: () => {
            ...
            router.delete(`/category/${id}`, {
                onSuccess: () => toast.add({ severity: "success", summary: "Deleted", detail: "Product deleted successfully!", life: 3000 }),
                onError: () => toast.add({ severity: "error", summary: "Error", detail: "Failed to delete product!", life: 3000 }),
            });
        },
